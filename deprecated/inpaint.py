...
def perturb(x, noise_type="normal", noise_level=0.5):
    h, w = x.shape[2:]
    mask = torch.zeros_like(x)
    h_start, h_end = h // 20, 19 * h // 20
    w_start, w_end = w // 20, 19 * w // 20
    mask[:, :, h_start:h_end, w_start:w_end] = 1.0

    if noise_type == "normal":
        # x = (1-mask) * x + mask * (noise_level * torch.randn_like(x) + (1-noise_level) * x)
        x = x + mask * noise_level * torch.randn_like(x)
...
def main(args):
    ...
    if rank == 0:
        subfolder_name = f"model{args.model}-cfg{args.cfg_scale}-step{args.stepsize}-nsteps{args.num_sampling_steps}-noise{args.noise_level}"
    ...
    for i in pbar:
        with torch.no_grad():
            ...
            # class 2
            noised = perturb(xt, "normal", args.noise_level)
            z = noised.clone()
            ...
            samples_inter = decode(intermediate)
            samples_noised = decode(noised)
            samples_final = decode(xt)
            ...
            for i, grid in enumerate(zip(samples_inter, samples_noised, samples_final)):
                index = i * dist.get_world_size() + rank + total
                out_path = f"{args.folder}/{index:06d}.png"
                save_image_grid(grid, out_path, nrows=1, padding=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default=None, help="Optional path to a custom EqM checkpoint")
