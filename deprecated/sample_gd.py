...
def save_gradient_norm_statistics(gradient_norms, args, folder):
    ...
    plt.fill_between(
        steps, gradient_means - gradient_stds, gradient_means + gradient_stds, alpha=0.3, label="Mean ± Std"
    )
    plt.xlabel("Sampling Step", fontsize=12)
    plt.ylabel("Gradient L2 Norm", fontsize=12)
    plt.title(f"Gradient L2 Norm during Sampling ({args.sampler.upper()}, stepsize={args.stepsize})", fontsize=14)
        ...
        with torch.no_grad():
            ...
            for step_idx in range(args.num_sampling_steps - 1):
                ...
                if step_idx in save_steps_list:
                    ...
                    samples_intermediate = (
                        torch.clamp(127.5 * samples_intermediate + 128.0, 0, 255)
                        .permute(0, 2, 3, 1)
